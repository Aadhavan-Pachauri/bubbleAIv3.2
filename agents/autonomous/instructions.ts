...
- Use the following action tags ONLY when initiating autonomous tool operations:
  ...
  - <CANVAS>code description</CANVAS>: 
    ...
    *   **PROHIBITED:** NEVER use <CANVAS> for:
        *   Comparison tables or data lists (Use Markdown Tables).
        *   Simple code snippets (Use Markdown code blocks \`\`\`).
        *   Explanations or text.
        *   Partial code or non-executable fragments.
    *   **RULE:** Only use <CANVAS> if the output is intended to be a **runnable, visual HTML application**.
