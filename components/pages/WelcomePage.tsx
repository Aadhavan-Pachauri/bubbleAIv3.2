...
type Page = 'home' | 'features' | 'technology' | 'pricing';
...
const pricingTiers = [
    { name: "Free", price: "$0", description: "For individuals getting started.", features: ["Daily Credit Allowance", "Autonomous Agent", "Standard AI Models", "Community Access"], popular: false, action: "Get Started" },
    { name: "Pro", price: "$10", description: "For professionals and power users.", features: ["Larger Credit Allowance", "Co-Creator Workspace", "Advanced AI Models", "Project Memory", "Priority Support"], popular: true, action: "Vote for Pro" },
    { name: "Max", price: "$25", description: "For teams and advanced creators.", features: ["Highest Credit Allowance", "All Pro Features", "Team Collaboration", "Expert-Level Agents", "Early Access to Features"], popular: false, action: "Vote for Max" }
];
...
const HomePage: React.FC<{ onAuthClick: () => void, onNavigate: (page: Page) => void }> = ({ onAuthClick, onNavigate }) => {
    ...
    return (
        <motion.div 
            ...
            animate={{ opacity: 1 }} 
            exit={{ opacity: 0 }}
            className="flex flex-col items-center pt-20 pb-20 px-6 min-h-[80vh] justify-center"
        >
            ...
            >
                <div className="relative aspect-[16/9] bg-black/40 rounded-xl border border-white/10 backdrop-blur-sm overflow-hidden shadow-2xl group cursor-pointer" onClick={() => onNavigate('features')}>
                    <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent z-10 flex flex-col justify-end p-8">
                        <div className="flex items-center gap-3 mb-2">
                            <span className="px-3 py-1 rounded-full bg-primary-start/20 text-primary-start border border-primary-start/30 text-xs font-bold uppercase tracking-wider">Showcase</span>
                            <span className="text-white/60 text-sm">Roblox Game Generation</span>
                        </div>
                        <h3 className="text-2xl font-bold text-white group-hover:text-primary-start transition-colors">See what Bubble can build in seconds &rarr;</h3>
                    </div>
                    {/* Abstract UI representation */}
                    <div className="absolute inset-0 grid grid-cols-12 grid-rows-6 gap-4 p-8 opacity-50 group-hover:opacity-70 transition-opacity">
                        <div className="col-span-3 row-span-6 bg-white/5 rounded-lg border border-white/5"></div>
                        <div className="col-span-9 row-span-4 bg-white/5 rounded-lg border border-white/5"></div>
                        <div className="col-span-9 row-span-2 bg-white/5 rounded-lg border border-white/5 flex items-center px-4">
...
const FeaturesPage: React.FC = () => (
    ...
    >
        ...
        <SectionHeader 
            ...
            subtitle="Whether you're a solo developer or leading a team, Bubble adapts to your workflow."
        />
    </motion.div>
);
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center mb-32">
            <div className="space-y-8">
                <div className="flex gap-4 items-start">
                    ...
                    <div>
                        <h3 className="text-xl font-bold text-white mb-2">Multi-Model Routing</h3>
                        <p className="text-gray-400">
                            Bubble dynamically selects the best model for the task. Coding? It might route to <strong>Claude 3.5 Sonnet</strong> or <strong>Gemini 3 Pro</strong>. Creative writing? <strong>DeepSeek V3</strong> or <strong>Llama 3.1</strong>. You are never locked in.
                        </p>
                    </div>
                </div>
                <div className="flex gap-4 items-start">
                    ...
                    <div>
                        <h3 className="text-xl font-bold text-white mb-2">Deep Research Agents</h3>
                        <p className="text-gray-400">
                            Powered by <strong>Tavily</strong> and <strong>Google Grounding</strong>, our research agents browse the live web, read documentation, and verify facts in real-time.
                        </p>
                    </div>
                </div>
                <div className="flex gap-4 items-start">
                    ...
                    <div>
                        <h3 className="text-xl font-bold text-white mb-2">Open Router Integration</h3>
                        <p className="text-gray-400">
                            Bring your own keys or use our managed fleet. Access an infinite library of open-source and proprietary models through our robust OpenRouter integration.
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-black/40 border border-white/10 rounded-3xl p-8 backdrop-blur-md relative overflow-hidden">
                <div className="absolute inset-0 bg-gradient-to-br from-primary-start/5 via-transparent to-primary-end/5 pointer-events-none"></div>
                <h3 className="text-sm font-bold text-gray-500 uppercase tracking-wider mb-6">Built With</h3>
                <div className="grid grid-cols-2 gap-4">
                    {['Google Gemini', 'OpenRouter', 'Anthropic', 'DeepSeek', 'Meta Llama', 'Tavily', 'Supabase'].map((tech) => (
                        <div key={tech} className="p-4 bg-white/5 border border-white/10 rounded-xl flex items-center justify-center text-center hover:bg-white/10 transition-colors">
...
const PricingPage: React.FC<{ onAuthClick: () => void }> = ({ onAuthClick }) => {
    ...
    const handleAction = (tier: string) => {
        if (tier === 'Free') {
            ...
            addToast(`Vote recorded for ${tier} Plan!`, 'success');
        }
    };
            ...
            <div className="grid md:grid-cols-3 gap-8 items-stretch mb-32">
                {pricingTiers.map((tier, index) => (
                    ...
                    >
                        ...
                        <button onClick={() => handleAction(tier.name)} className={`w-full py-4 font-bold rounded-xl transition-all ${tier.popular ? 'bg-primary-start text-white hover:bg-primary-start/80 shadow-lg hover:shadow-primary-start/20' : 'bg-white text-black hover:bg-gray-100'}`}>
                            {tier.action}
                        </button>
                    </motion.div>
                ))}
        ...
        >
            <div className="max-w-7xl mx-auto bg-black/50 backdrop-blur-xl border border-white/10 rounded-full px-6 py-3 flex justify-between items-center shadow-2xl">
                ...
                <nav className="hidden md:flex items-center gap-1 bg-white/5 rounded-full p-1 border border-white/5">
                    {(['home', 'features', 'technology', 'pricing'] as Page[]).map((page) => (
                        <button
                            key={page}
...
export const WelcomePage: React.FC = () => {
  ...
  return (
    <div className="relative min-h-screen w-full text-white bg-bg-primary overflow-x-hidden selection:bg-primary-start/30 selection:text-white">
        <AuroraCanvas />
        <div className="absolute inset-0 -z-10 bg-black/40"></div> {/* Overlay to ensure text readability */}
