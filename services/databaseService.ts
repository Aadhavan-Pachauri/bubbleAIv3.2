...
export const sendFriendRequest = async (supabase: SupabaseClient, senderId: string, receiverId: string): Promise<void> => {
    const { error } = await supabase
        .from('friendships')
...
export const getFriendships = async (supabase: SupabaseClient, userId: string): Promise<any[]> => {
    ...
    return (data || []).map((f: any) => ({
        ...
        other_user: f.user_id === userId ? f.friend : f.user
    }));
};
...
export const extractAndSaveMemory = async (
    ...
    // For non-guests, if no key provided, fetch from DB
    if (!apiKey && userId !== 'guest') {
    ...
    if (!apiKey) {
        // Without an API key (guest without key, or user without key), we cannot perform extraction
        return; 
    }
...
export const loadMemoriesForPrompt = async (supabase: SupabaseClient, userId: string, prompt: string, projectId?: string | null): Promise<string> => {
    ...
    const relevantMemories = data.filter(m => {
        ...
        if (layer === 'personal' || layer === 'aesthetic') {
            return true;
        }
        
        // Rule 2: If we are in a project, include memories specific to that project.
        if (projectId && (layer === 'project' || layer === 'codebase')) {
            const memoryProjectId = metadata?.project_id;
    ...
    finalMemories.forEach(m => {
        ...
        const value = m.content || m.value;
        const layer = m.layer;
        
        if (layer && key && value) {
           const targetLayer = byLayer[layer] ? layer : 'personal';
...
export const deleteMemory = async (supabase: SupabaseClient, memoryId: string): Promise<void> => {
    ...
    if (memoryId.startsWith('guest-mem-')) {
        const memories = JSON.parse(localStorage.getItem('guest_memories') || '[]');
        const filtered = memories.filter((m: any) => m.id !== memoryId);
        localStorage.setItem('guest_memories', JSON.stringify(filtered));
        return;
    }
